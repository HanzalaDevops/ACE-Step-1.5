...
class ConditioningTargetMixin:
            ...
            with self._load_model_context("vae"):
                ...
                for i in range(batch_size):
                    ...
                    if source_repaint_batch is not None:
                        ...
                        logger.info(
                            "[generate_music] Using session source latents for repaint item {}...",
                            i,
                        )
    ...
    def _normalize_source_repaint_latents(
        ...
        """Normalize optional session source latents to ``[B, T, C]``."""
        if source_repaint_latents is None:
            return None
        latents = torch.as_tensor(
            source_repaint_latents,
            device=self.device,
            dtype=self._get_vae_dtype(),
        )
        if latents.ndim == 2:
    ...
    def _align_source_repaint_latent(
        ...
        """Trim or silence-pad a session source latent to the source-audio length."""
        latent = latent.to(device=self.device, dtype=self._get_vae_dtype())
